...
使用方法:
    python collect_g1_dds_data_with_force.py <output_dir> [--duration 30] [--domain-id 0] [--dof-mode 12dof|29dof|27dof]
"""
...
示例:
  # 采集30秒数据到指定目录
  python collect_g1_dds_data_with_force.py ./data/varied_scenes/var_0000 --duration 30

  # 采集并导出 29DoF 数据
  python collect_g1_dds_data_with_force.py ./data/var_0029 --duration 30 --dof-mode 29dof

  # 兼容旧的 lock-waist / 27DoF 数据
  python collect_g1_dds_data_with_force.py ./data/var_0027 --duration 30 --dof-mode 27dof

  # 使用不同的domain_id
  python collect_g1_dds_data_with_force.py ./data/test --duration 20 --domain-id 1

  # 只处理已有的CSV文件
  python collect_g1_dds_data_with_force.py ./data/var_0000 --process-only raw_data_xxx.csv
"""
    )
