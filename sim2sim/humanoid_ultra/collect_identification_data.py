...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=31,
        help="对 dq 做 Savitzky-Golay 滤波微分得到 ddq 的窗口长度(与实机管线一致，限带去冲击)。"
             "设为 1 则保留 mujoco 原始 qacc(满带宽，含冲击尖峰，不推荐用于辨识)。",
    )
    parser.add_argument(
        "--elastic-band",
        action="store_true",
...
class IdentificationRecorder:
    ...
    def recompute_ddq_savgol(self, window: int = 31, polyorder: int = 2) -> None:
        ...
        """
        if window <= 1:
            return  # 关闭 savgol，保留 mujoco 原始 qacc
        if self.ddq.shape[1] < window:
            print(f"  [savgol] 样本数 {self.ddq.shape[1]} < window {window}，跳过 ddq 重算")
            return
        dt = SAMPLE_DT
        # rows 0-2: base 线加速度(IMU) 保留；rows 3-5: base 角加速度；rows 6..: 关节
        self.ddq[3:] = savgol_filter(
            self.dq[3:], window_length=window, polyorder=polyorder,
            deriv=1, delta=dt, axis=1,
        )
        self._ddq_savgol_window = window
        print(f"  [savgol] ddq(关节+base角) 已用 savgol(window={window}, poly={polyorder}) 重算"
              f"；base 线加速度保留 IMU")
...
def write_metadata(
    ...
    metadata = {
        ...
        "ddq": {
            ...
            "source": (
                "joint ddq & base angular accel = Savitzky-Golay derivative of dq "
                f"(window={args.ddq_savgol_window}, polyorder=2); base linear accel = IMU accelerometer"
                if args.ddq_savgol_window > 1 else
                "MuJoCo acceleration-stage ground truth; no numerical differentiation"
            ),
            "ddq_savgol_window": args.ddq_savgol_window,
        },
        "tau": {
            "unit": "Nm",
...
def main() -> None:
    ...
    recorder.recompute_ddq_savgol(window=args.ddq_savgol_window)
